...
#define EXT2_FIRST_INO(s)                                               \
    (((s)->sb_rev_level == EXT2_GOOD_OLD_REV) ? EXT2_GOOD_OLD_FIRST_INO \
                                              : (s)->sb_first_ino)
...
/* size with padding */
#define EXT2_DIR_ENTRY_ACTUAL_SIZE(d)               \
    (EXT2_DIR_ENTRY_CONTENTS_SIZE(d) +              \
     ((EXT2_DIR_ENTRY_CONTENTS_SIZE(d) & 0x03) == 0 \
          ? 0                                       \
          : EXT2_DIR_ENTRY_ALIGN - (EXT2_DIR_ENTRY_CONTENTS_SIZE(d) & 0x03)))

/* How many bytes can be taken from the end of dentry */
#define EXT2_DIR_ENTRY_SHRINK(d) \
    ((d)->d_rec_len - EXT2_DIR_ENTRY_ACTUAL_SIZE(d))
