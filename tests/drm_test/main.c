...
static int drmmodeset_prepare(int fd)
{
    ...
    for (i = 0; i < res->count_connectors; ++i) {
        ...
        if (!conn) {
            fprintf(stderr, "cannot retrieve DRM connector %u:%lu (%d): %s\n",
        ...
        if (ret) {
            if (ret != -ENOENT) {
                errno = -ret;
                fprintf(stderr,
                        "cannot setup device for connector %u:%lu (%d): %s\n",
                        i, res->connectors[i], errno, strerror(errno));
            }
            free(dev);
                                ...
                                struct drmmodeset_dev* dev)
{
    ...
    if (conn->connection != DRM_MODE_CONNECTED) {
        fprintf(stderr, "ignoring unused connector %lu\n", conn->connector_id);
        return -ENOENT;
    }

    /* check if there is at least one valid mode */
    if (conn->count_modes == 0) {
        fprintf(stderr, "no valid mode for connector %lu\n",
                conn->connector_id);
        return -EFAULT;
    }
    ...
    fprintf(stderr, "mode for connector %lu is %lux%lu\n", conn->connector_id,
            dev->width, dev->height);

    /* find a crtc for this connector */
    ret = drmmodeset_find_crtc(fd, res, conn, dev);
    if (ret) {
        fprintf(stderr, "no valid crtc for connector %lu\n",
                conn->connector_id);
        return ret;
    }
    ...
    if (ret) {
        fprintf(stderr, "cannot create framebuffer for connector %lu\n",
                conn->connector_id);
        return ret;
    }
                                ...
                                struct drmmodeset_dev* dev)
{
    ...
    for (i = 0; i < conn->count_encoders; ++i) {
        ...
        if (!enc) {
            fprintf(stderr, "cannot retrieve encoder %u:%lu (%d): %s\n", i,
                    conn->encoders[i], errno, strerror(errno));
            continue;
        }
    ...
    fprintf(stderr, "cannot find suitable CRTC for connector %lu\n",
            conn->connector_id);
    return -ENOENT;
}
...
int main(int argc, char** argv)
{
    ...
    for (iter = drmmodeset_list; iter; iter = iter->next) {
        ...
        ret = drmModeSetCrtc(fd, iter->crtc, iter->fb, 0, 0, &iter->conn, 1,
                             &iter->mode);
        if (ret)
            fprintf(stderr, "cannot set CRTC for connector %lu (%d): %s\n",
...
out_return:
    if (ret) {
        errno = -ret;
        fprintf(stderr, "drmmodeset failed with error %d: %s\n", errno,
                strerror(errno));
    } else {
