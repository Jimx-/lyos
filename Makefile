...
kvm:
	@qemu-system-x86_64 -machine q35 -smp 2 -kernel $(LYOSKERNEL) -append "console=ttyS0 video=1024x768 watchdog=1" -initrd "$(LYOSINITRD),$(DESTDIR)/sbin/mm,$(DESTDIR)/sbin/pm,$(DESTDIR)/sbin/servman,$(DESTDIR)/sbin/devman,$(DESTDIR)/sbin/sched,$(DESTDIR)/sbin/vfs,$(DESTDIR)/sbin/systask,$(DESTDIR)/sbin/tty,$(DESTDIR)/sbin/ramdisk,$(DESTDIR)/sbin/initfs,$(DESTDIR)/sbin/sysfs,$(DESTDIR)/sbin/ipc,$(DESTDIR)/sbin/netlink,$(DESTDIR)/sbin/init" -drive id=hda,file=lyos-disk-$(SUBARCH).img,format=raw,if=none -device virtio-blk-pci,drive=hda -netdev user,id=net0,hostfwd=tcp::5555-:22 -device virtio-net,netdev=net0 -object filter-dump,id=f1,netdev=net0,file=dump.dat -device pci-ohci -device usb-kbd -device usb-mouse -m 2048 -serial stdio -vga virtio -display sdl -cpu host,pmu=true --enable-kvm
