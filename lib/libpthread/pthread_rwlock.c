...
static int __can_rdlock(int state)
{
...
static int __pthread_rwlock_timedrdlock(pthread_rwlock_t* lock, const struct timespec* abs_timeout)
{
    /*
...
static int __pthread_rwlock_timedwrlock(pthread_rwlock_t* lock, const struct timespec* abs_timeout)
{
    /*
    if (__atomic_load_n(&lock->owner, __ATOMIC_RELAXED) == pthread_self()) {
        return EDEADLK;
...
int pthread_rwlock_unlock(pthread_rwlock_t* lock)
{
    ...
    if (RW_WRLOCKED(old_state)) {
        /* if (pthread_self() != __atomic_load_n(&lock->owner, __ATOMIC_RELAXED)) return EPERM; */
