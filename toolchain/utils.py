...
def configure(name, extra_opt=''):
    ...
    os.system(path + ' --target=' + TARGET + ' --prefix=' + PREFIX + ' ' +
              ' --with-sysroot=' + SYSROOT + extra_opt) and cmd_error()


def configure_cross(name, extra_opt=''):
    path = os.sep.join([ROOT_DIR, 'sources', name, 'configure'])
    os.system(path + ' --target=' + TARGET + ' --prefix=' + CROSSPREFIX +
              extra_opt) and cmd_error()


def configure_host(name, extra_opt=''):
    path = os.sep.join([ROOT_DIR, 'sources', name, 'configure'])
    os.system(path + ' --host=' + TARGET + ' --prefix=' + CROSSPREFIX + ' ' +
              extra_opt) and cmd_error()


def configure_native(name, extra_opt='', libpath=True):
    path = os.sep.join([ROOT_DIR, 'sources', name, 'configure'])
    if libpath:
        libpath_opt = ' --with-lib-path=' + PREFIX + '/' + TARGET + '/lib '
    else:
        libpath_opt = ' '
