...
{
    int irq = m->IRQ_IRQ, hook_id = m->IRQ_HOOK_ID - 1, notify_id = m->IRQ_HOOK_ID;
    int retval = 0, i;
    ...
    switch (m->IRQ_REQUEST) {
        case IRQ_ENABLE:
        case IRQ_DISABLE:
            if (hook_id < 0 || hook_id >= NR_IRQ_HOOKS || irq_hooks[hook_id].proc_ep == NO_TASK) return EINVAL;
            if (irq_hooks[hook_id].proc_ep != p_proc->endpoint) return EPERM;
            if (m->IRQ_REQUEST == IRQ_ENABLE) enable_irq(&irq_hooks[hook_id]);
            else disable_irq(&irq_hooks[hook_id]);
            break;
        case IRQ_SETPOLICY:
