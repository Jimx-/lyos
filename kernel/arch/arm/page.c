...
PRIVATE phys_bytes kern_phys_base
    __attribute__((__section__(".unpaged_data"))) = (phys_bytes)&_PHYS_BASE;
                   ...
                   kinfo_t* pk)
{
    ...
    if (vir_addr % ARCH_PG_SIZE)
        vir_addr = (vir_addr / ARCH_PG_SIZE) * ARCH_PG_SIZE;
    ...
    while (vir_addr < vir_end) {
        ...
        if (pgd[pde] & ARM_VM_SECTION) {
            ...
            pgd[pde] =
                (pt_ph & ARM_VM_PDE_MASK) | ARM_VM_PAGEDIR | ARM_VM_PDE_DOMAIN;
        } else {
            pt = (pte_t*)(((phys_bytes)pgd[pde] & ARCH_PG_MASK) +
                          (phys_bytes)&_KERN_OFFSET);
        }

        pt[pte] =
            (phys & ARM_PG_MASK) | ARM_PG_PRESENT | ARM_PG_CACHED | ARM_PG_USER;
...
PUBLIC void pg_identity(pde_t* pgd)
{
    ...
    for (i = 0; i < ARCH_VM_DIR_ENTRIES; i++) {
        int flags =
            ARM_VM_SECTION | ARM_VM_SECTION_USER | ARM_VM_SECTION_DOMAIN;
...
PUBLIC void pg_mapkernel(pde_t* pgd)
{
    ...
    while (mapped_size < kern_size) {
        pgd[pde] = (kern_phys & ARM_VM_SECTION_MASK) | ARM_VM_SECTION |
